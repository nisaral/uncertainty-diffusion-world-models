...
| Multi-worker merge race (infra) | FIXED (2026-09-10) | The policy split driver (`run_policy_2x2_split_seeds.py`, behind every DMC/Walker/MACURA/Kaggle run) merged by *overwriting* `--out`: two invocations sharing one `--out` meant last-writer-wins, silently dropping the other's rows and then deleting the partials it never merged - the same class of silent corruption as the earlier `--set` precedence bug. Merge is now a shared, single-implementation race-safe utility (`udwm/utils/atomic_merge.py`): exclusive lock -> read-existing -> union -> assert every requested (seed, arm) present -> atomic tmp+rename -> only then remove partials; protocol guard refuses to union rows from a different config/steps/arm set. Both probes were refactored onto the same utility (they were the only two files that already had the fix - `run_policy_2x2_split_seeds.py` was the one place the bug was still live). 7 new tests (`tests/test_atomic_merge.py`); full suite 59 passing |
| Run status / row-count sanity tool | ADDED (2026-09-10) | `udwm/scripts/run_status.py` prints, for a running or finished study, which (seed, arm) rows are on disk, the expected `seeds x arms` total and the exact missing list, a COMPLETE/INCOMPLETE verdict, a rough ETA, and a stale-write flag - i.e. the row-count assertion made operable for a human at the terminal. Reads JSON only, no GPU; handles both partial shapes (split-driver per-seed payloads, probe single-row files) and names the right summarizer per study. 5 tests in `tests/test_run_status.py` |
